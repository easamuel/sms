...
@section('title', 'Practice Session - ' . $practiceSession->title)
        ...
        <div class="sms-practice-card">
            <div class="sms-practice-header">
                ...
                <span class="sms-practice-badge">
                    <i class="fas fa-dumbbell"></i> Practice Session
                </span>
            </div>
            ...
            <form method="POST" action="{{ route('sms.student.practice-sessions.submit', $practiceSession->id) }}" id="practiceForm">
                @csrf
                ...
                <div class="sms-question-card" data-question-id="{{ $question->id }}">
                    ...
                    <div class="sms-options-list">
                        ...
                        <div class="sms-option-item" data-option="{{ $option }}" onclick="selectOption(this, {{ $question->id }}, '{{ $option }}')">
                            <div class="sms-option-radio"></div>
                            <div class="sms-option-text">{{ chr(65 + $key) }}. {{ $option }}</div>
                        </div>
                        @endforeach
                    </div>
                    <input type="hidden" name="answers[{{ $question->id }}]" id="answer_{{ $question->id }}" value="">
                    @else
                    <div style="padding: 1rem; background: white; border-radius: 8px; border: 1px solid var(--sms-gray-200); margin-top: 1rem;">
                        <textarea 
                            name="answers[{{ $question->id }}]" 
                            rows="4" 
                            style="width: 100%; border: none; outline: none; resize: vertical; font-size: 0.9375rem;"
                            placeholder="Type your answer here..."
                        >{{ $attempt->answers[$question->id] ?? '' }}</textarea>
                    </div>
                    @endif
                </div>
                @endforeach
                ...
                <div class="sms-actions">
                    <a href="{{ route('sms.student.practice-sessions') }}" class="sms-btn sms-btn-secondary">
...
function selectOption(element, questionId, option) {
    ...
    questionCard.querySelectorAll('.sms-option-item').forEach(item => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const savedAnswer{{ $qId }} = '{{ $answer }}';
    const questionCard{{ $qId }} = document.querySelector('[data-question-id="{{ $qId }}"]');
    if (questionCard{{ $qId }}) {
        const optionItem = questionCard{{ $qId }}.querySelector(`[data-option="${savedAnswer{{ $qId }}}"]`);
        if (optionItem) {
            optionItem.classList.add('selected');
            document.getElementById('answer_{{ $qId }}').value = savedAnswer{{ $qId }};
        }
    }
    @endif
    @endforeach
