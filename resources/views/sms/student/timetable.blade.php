...
<head>
    ...
    <title>Class Routine &amp; Timetable - ES-SCHOOLS Student Portal</title>
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <ul class="sidebar-menu">
            <li class="menu-item">
                <a href="{{ route('sms.student.dashboard') }}" class="menu-link">
                    <div class="menu-left">
            ...
            <li class="menu-item">
                <a href="{{ route('sms.student.timetable') }}" class="menu-link active">
                    <div class="menu-left">
    ...
    <div class="main-wrapper">
        ...
        <header class="top-header">
            <div class="header-left">
                ...
                <a href="{{ route('sms.student.dashboard') }}" class="home-btn" title="Dashboard Home">
                    <i class="fas fa-home"></i>
                </a>
                <div style="font-size: 0.88rem; font-weight: 700; color: var(--text-muted);">
                    <a href="{{ route('sms.student.dashboard') }}" style="color: var(--text-muted); text-decoration: none;">Dashboard</a>
                    <span style="margin: 0 0.35rem;">/</span>
                    <span style="color: var(--primary);">Class Routine</span>
                </div>
            </div>
            ...
            <div class="header-right">
                ...
                <div class="profile-pill">
                    <div class="profile-avatar">
                        {{ strtoupper(substr($student->user->first_name ?? ($student->user->name ?? 'S'), 0, 1)) }}
                    </div>
                    <div class="brand-text-wrap" style="text-align: left;">
                        <span class="profile-name">{{ $student->user->full_name ?? ($student->user->name ?? 'Demo Student') }}</span>
                        <span class="profile-role">{{ $student->class->name ?? 'Student' }}</span>
                    </div>
                </div>
            </div>
        </header>
        ...
        <main class="content-container">
            ...
            <div class="page-header-row">
                <div>
                    ...
                    <p class="page-subtitle">
                        Weekly lecture periods, classroom locations, and teachers for {{ $student->class->name ?? 'Class' }} ({{ $currentYear ?? '2026/2027' }}).
                    </p>
                </div>
                <div>
            ...
            <div class="routine-stats-grid">
                <div class="stat-card">
                    ...
                    <div>
                        <div class="stat-title">My Class</div>
                        <div class="stat-val">{{ $student->class->name ?? 'Junior Sec 2' }}</div>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div>
                        ...
                        <div class="stat-val">{{ $timetables->count() ?? 15 }} Classes</div>
                    </div>
                </div>
            ...
            <div class="day-schedule-card day-section-item" id="day-section-{{ $dayKey }}">
                <div class="day-schedule-header">
                    ...
                    <span class="day-count-badge">
                        {{ $daySlots->count() }} {{ \Illuminate\Support\Str::plural('Period', $daySlots->count()) }}
                    </span>
                </div>
                ...
                <div style="padding: 2rem; text-align: center; color: var(--text-muted);">
                    ...
                    <p style="font-size: 0.88rem; font-weight: 600;">No classes scheduled for {{ $dayLabel }}.</p>
                    <span style="font-size: 0.78rem;">Enjoy personal study time, library research, or extracurriculars.</span>
                </div>
                @else
                <!-- Desktop Table View -->
                    ...
                    <table class="routine-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="subject-tag">{{ $slot->subject->name ?? 'General Science' }}</span>
                                    <span style="font-size: 0.75rem; color: var(--text-muted);">Code: {{ $slot->subject->code ?? 'SCI101' }}</span>
                                </td>
                                <td>
                                    <div class="teacher-tag">
                                        <i class="fas fa-chalkboard-teacher" style="color: var(--primary);"></i>
                                        <span>{{ $slot->teacher->user->full_name ?? ($slot->teacher->user->name ?? 'Faculty Instructor') }}</span>
                                    </div>
                                </td>
                                <td>
                                    <strong style="color: var(--text-main);">{{ $slot->class->name ?? 'Room 204' }}</strong>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                ...
                <div class="mobile-routine-card">
                    ...
                    <h4 style="font-size: 0.95rem; font-weight: 800; color: var(--text-main); margin-bottom: 0.2rem;">
                        {{ $slot->subject->name ?? 'General Science' }}
                    </h4>
                    <div style="font-size: 0.8125rem; color: var(--text-muted); display: flex; align-items: center; gap: 0.35rem; margin-top: 0.35rem;">
                        <i class="fas fa-chalkboard-teacher" style="color: var(--primary);"></i>
                        <span>{{ $slot->teacher->user->full_name ?? ($slot->teacher->user->name ?? 'Faculty Instructor') }}</span>
                    </div>
                </div>
