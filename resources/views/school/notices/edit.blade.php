...
@section('title', 'Edit Notice - School Management System')
...
<div class="sms-page">
    <div class="sms-page-header">
        <h1 class="sms-page-title">Edit Notice</h1>
    </div>

    <div class="sms-card">
        <div class="sms-card-body">
            <form method="POST" action="{{ route('school.notices.update', $notice) }}">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <textarea name="content" class="form-control" required>{{ old('content', $notice->content) }}</textarea>
                    @error('content')
                        <p class="text-danger text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="target_audience" class="form-control" required>
                        <option value="all" {{ old('target_audience', $notice->target_audience) == 'all' ? 'selected' : '' }}>All Users</option>
                        <option value="parents" {{ old('target_audience', $notice->target_audience) == 'parents' ? 'selected' : '' }}>Parents Only</option>
                        <option value="students" {{ old('target_audience', $notice->target_audience) == 'students' ? 'selected' : '' }}>Students Only</option>
                        <option value="teachers" {{ old('target_audience', $notice->target_audience) == 'teachers' ? 'selected' : '' }}>Teachers Only</option>
                    </select>
